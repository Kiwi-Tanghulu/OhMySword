...
namespace Server
{
    public class RoomManager
    {
        ...
        public GameRoom GetRoom()
        {
            ...
            PublishRoomID(room);
            Console.WriteLine($"{DateTime.Now.ToString("yy년 MM월 dd일 HH:mm:ss")} [Room] Room Created {{ Room ID : {room.RoomID} }}");
        ...
        public void ReleaseRoom(GameRoom room)
        {
            ...
            room.AddJob(() => {
                room.Clear();
                Console.WriteLine($"{DateTime.Now.ToString("yy년 MM월 dd일 HH:mm:ss")} [Room] Room Destroyed {{ Room ID : {room.RoomID} }}");
