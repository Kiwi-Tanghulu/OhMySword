...
public class PlayerControl : MonoBehaviour
{
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawLine(new Vector3(hips.position.x + leftFootOffset.x, hips.position.y, hips.position.z + leftFootOffset.z), leftHitPos);
        Gizmos.DrawLine(new Vector3(hips.position.x + rightFootOffset.x, hips.position.y, hips.position.z + rightFootOffset.z), rightHitPos);

        Gizmos.color = Color.red;
        Gizmos.DrawWireSphere(leftFootPos, shouldMoveDistance * 1.7f);
        Gizmos.DrawWireSphere(rightFootPos, shouldMoveDistance);

        Gizmos.color = Color.blue;
        Gizmos.DrawSphere(leftHitPos, 0.05f);
        Gizmos.DrawSphere(rightHitPos, 0.05f);
    }
#endif
}
